import mujoco
import mujoco.viewer
import numpy as np
import time
import socket
import json
import argparse
from state_extractor import SegwayStateExtractor
from lqr_controller import SegwayLQR

MODEL_PATH = "segway.xml"
SIM_DT = 0.002
TORQUE_LIMIT = 20.0
UDP_STATE_PORT = 9091
UDP_TORQUE_PORT = 9092

class SegwaySimulation:
    def __init__(self):
        self.model = mujoco.MjModel.from_xml_path(MODEL_PATH)
        self.data = mujoco.MjData(self.model)
        self.ext = SegwayStateExtractor(self.model)
        self.lqr = SegwayLQR(torque_limit=TORQUE_LIMIT)
        self.L_act = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, "L_wheel_torque")
        self.R_act = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, "R_wheel_torque")
        self.state_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.failed = False
        self.log_data = []

    def reset(self, pitch_deg=2.0):
        mujoco.mj_resetData(self.model, self.data)
        pitch = np.deg2rad(pitch_deg)
        self.data.qpos[3] = np.cos(pitch/2)
        self.data.qpos[5] = np.sin(pitch/2)
        self.failed = False
        self.log_data = []
        mujoco.mj_forward(self.model, self.data)

    def step(self):
        state = self.ext.get_state(self.data)
        if abs(state[0]) > np.deg2rad(30):
            self.failed = True
            self.data.ctrl[self.L_act] = 0
            self.data.ctrl[self.R_act] = 0
            mujoco.mj_step(self.model, self.data)
            return state, 0, 0
        tL, tR = self.lqr.compute_torque(state)
        self.data.ctrl[self.L_act] = tL
        self.data.ctrl[self.R_act] = tR
        mujoco.mj_step(self.model, self.data)
        return state, tL, tR

    def send_state_udp(self, state, tL, tR):
        msg = json.dumps({"time": round(self.data.time,4),
            "theta": round(float(state[0]),6), "theta_dot": round(float(state[1]),6),
            "phi": round(float(state[2]),6), "phi_dot": round(float(state[3]),6),
            "tau_L": round(float(tL),4), "tau_R": round(float(tR),4)}).encode()
        try: self.state_sock.sendto(msg, ("127.0.0.1", UDP_STATE_PORT))
        except: pass

    def run_viewer(self, pitch_deg=2.0):
        self.reset(pitch_deg)
        count = 0
        with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
            while viewer.is_running():
                t0 = time.time()
                state, tL, tR = self.step()
                count += 1
                if count % 5 == 0: self.send_state_udp(state, tL, tR)
                if count % 500 == 0: self.ext.print_state(self.data)
                viewer.sync()
                dt = SIM_DT - (time.time() - t0)
                if dt > 0: time.sleep(dt)

    def run_headless(self, duration=10.0, pitch_deg=2.0):
        self.reset(pitch_deg)
        steps = int(duration / SIM_DT)
        for i in range(steps):
            state, tL, tR = self.step()
            self.log_data.append({"time": self.data.time, "theta": state[0],
                "theta_dot": state[1], "phi": state[2], "phi_dot": state[3],
                "tau_L": tL, "tau_R": tR})
            if i % 5 == 0: self.send_state_udp(state, tL, tR)
            if i % 500 == 0: self.ext.print_state(self.data)
            if self.failed: print(f"FAILED at t={self.data.time:.3f}"); break
        thetas = [abs(d["theta"]) for d in self.log_data]
        taus = [abs(d["tau_L"]) for d in self.log_data]
        print(f"\n[Result] steps={len(self.log_data)}, "
              f"theta_max={np.degrees(max(thetas)):.2f}deg, "
              f"tau_RMS={np.sqrt(np.mean(np.array(taus)**2)):.3f}Nm, "
              f"failed={self.failed}")

    def close(self):
        self.state_sock.close()

if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument("--headless", action="store_true")
    p.add_argument("--duration", type=float, default=10.0)
    p.add_argument("--pitch", type=float, default=2.0)
    args = p.parse_args()
    sim = SegwaySimulation()
    try:
        if args.headless: sim.run_headless(args.duration, args.pitch)
        else: sim.run_viewer(args.pitch)
    except KeyboardInterrupt: print("\nStopped")
    finally: sim.close()
